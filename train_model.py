...
from lib import environnement, data, model, common, validation
...
if __name__ == "__main__":
    ...
    if args.year is not None or os.path.isfile(args.data):
        ...
        env = environnement.StockEnvironment(stock_data, bars_count=BARS_COUNT, reset_on_close=True, volumes=False)
        env_tst = environnement.StockEnvironment(stock_data, bars_count=BARS_COUNT, reset_on_close=True)
    elif os.path.isdir(args.data):
        env = environnement.StockEnvironment(load_data(args.data, bars_count=BARS_COUNT, reset_on_close=True))
        env_tst = environnement.StockEnvironment(load_data(args.data, bars_count=BARS_COUNT, reset_on_close=True))
    else:
        raise RuntimeError("No data to train on")
    env = gym.wrappers.TimeLimit(env, max_episode_steps=1000)

    val_data = {"YNDX": data.load_relative(args.valdata)}
    env_val = environnement.StockEnvironment(val_data, bars_count=BARS_COUNT, reset_on_close=True)
